...
class AiChatServiceTest {
    ...
    void noResultsReturnsGroundedMessageWithoutCallingModel() {
        ...
        when(search.search(10L, "question", List.of(), 6)).thenReturn(List.of());
        ...
        service.answer(10L, "question", List.of(), true, emitter);
    ...
    void searchFailureEmitsSseError() {
        AiSearchService search = mock(AiSearchService.class);
        when(search.search(10L, "question", List.of(), 6)).thenThrow(new RuntimeException("Elasticsearch unavailable"));
        ChatClient.Builder builder = mock(ChatClient.Builder.class);
        when(builder.build()).thenReturn(mock(ChatClient.class));
        AiChatService service = new AiChatService(search, builder, mock(StringRedisTemplate.class), new ObjectMapper());
        CapturingEmitter emitter = new CapturingEmitter();

        service.answer(10L, "question", List.of(), false, emitter);
