...
async function avatar(event) {
  ...
  try { message.value = await api('/api/user/upload-avatar', { method: 'POST', body }); await load() }
  catch (error) { message.value = error.message }
}
</script>
